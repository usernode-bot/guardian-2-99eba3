...
// Memo format: { app: "guardian", type: "message|token_transfer", ... }
...
app.get('/api/user/messages', async (req, res) => {
  try {
    ...
    const { rows: countResult } = await pool.query(`
      SELECT COUNT(*) as total FROM messages WHERE sender_id = $1
    `, [userId]);
...
app.get('/api/activity', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(`
      ...
      FROM blockchain_audit_logs
      WHERE user_id = $1 AND message_type IN ('token_transfer')
      ORDER BY created_at DESC
      LIMIT $2 OFFSET $3
    `, [userId, limit, offset]);
